...
public class CommandChunkyPlayer implements ChunkyCommandExecutor {
    ...
    public void onCommand(CommandSender sender, ChunkyCommand command, String label, String[] args) {
        ...
        if (args.length == 0) {
            IChunkyPlayer chunkyPlayer = ChunkyManager.getChunkyPlayer((Player) sender);
            displayInfo(chunkyPlayer, sender);
            return;
        }
        if (args.length > 0) {
            IChunkyPlayer chunkyPlayer = ChunkyManager.getChunkyPlayer(args[0]);
    ...
    private void displayInfo(IChunkyPlayer chunkyPlayer, CommandSender sender) {
        ...
        Language.PLAYER_MENU_TITLE.normal(sender,
                ...
                sFirstDate,
                sLastDate,
                chunkyPlayer.getOwnables().get(IChunkyChunk.class.getName()).size(),
                chunkyPlayer.getChunkClaimLimit());
        HashSet<IChunkyObject> chunks = chunkyPlayer.getOwnables().get(IChunkyChunk.class.getName());

        if (chunks != null && chunks.size() > 0) {
            Language.PLAYER_MENU_OWNEDCHUNKS.normal(sender);
            String message = "";
            int i = 0;
            for (IChunkyObject o : chunks) {
                IChunkyChunk chunk = (IChunkyChunk) o;
    ...
    private void displayGlobalPermissions(IChunkyPlayer chunkyPlayer, CommandSender sender) {
        ...
        if (chunkyPlayer.getName().equals(sender.getName())) {
            HashMap<IChunkyObject, PermissionRelationship> playerPermissions = ChunkyManager.getAllPermissions(chunkyPlayer);
            String players = "";
            if (!playerPermissions.isEmpty()) {
                for (Map.Entry<IChunkyObject, PermissionRelationship> permPlayer : playerPermissions.entrySet()) {
                    if (permPlayer.getKey() instanceof IChunkyPlayer) {
                        IChunkyPlayer cPlayer = (IChunkyPlayer) permPlayer.getKey();
